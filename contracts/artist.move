module waltune::artist {
    ...
    struct ArtistRegistry has key {
        id: UID,
        artists: Table<address, address>, // wallet_address -> artist_object_id
        total_artists: u64,
    }
    ...
    public entry fun register(
        ...
        assert!(!table::contains(&registry.artists, sender), 0);
        ...
        let artist_address = object::uid_to_address(&artist.id);
        table::add(&mut registry.artists, sender, artist_address);
    ...
    public fun increment_songs(artist: &mut Artist) {
        artist.total_songs = artist.total_songs + 1;
    }

    /// Add earnings to artist (called by PaymentRouter)
    public fun add_earnings(artist: &mut Artist, amount: u64) {
