...
import {
  ...
  requirePayment,
  X402Request,
} from "../middleware/x402Middleware.js";
...
export default async function songRoutes(fastify: FastifyInstance) {
  ...
  fastify.get("/stream/:id", async (request: X402Request, reply) => {
    const { id } = request.params as { id: string };
    const { walrusBlobId: fallbackBlobId } = request.query as {
      walrusBlobId?: string;
    };
    ...
    try {
      ...
      if (!song) {
        console.warn(
          `⚠️ Song not found as object, trying to find in events...`
        );
        ...
        song = allSongs.find((s: any) => {
          ...
          return (
            s.id === id || s.id?.toLowerCase().startsWith(id.toLowerCase())
          );
        });
        ...
        if (song) {
          ...
          console.log(
            `   Walrus Blob ID: ${
              song.walrusCID || song.walrusBlobId || song.walrus_blob_id
            }`
          );
        } else {
          console.warn(
            `⚠️ Song not found in events either. Searched ID: ${id}`
          );
          console.log(
            `   Available song IDs (first 3):`,
            allSongs.slice(0, 3).map((s: any) => s.id)
          );
        }
      }
      ...
      if (!song && !finalBlobId) {
        console.error(
          `❌ Payment verified but song not found and no blob ID: ${id}`
        );
        return reply.code(404).send({
          error: "Song not found",
          message:
            "Payment verified but song not found in blockchain and no blob ID provided.",
          songId: id,
        });
      }
      ...
      if (song) {
        // Check if this was a play credits payment
        const x402Verified = (request as any).x402Verified;
        if (x402Verified?.paymentMethod === "play_credits") {
          // Use pay_for_play to transfer SUI from treasury to artist
          suiService
            .payForPlay(id, pricePerPlay)
            .then(() => {
              console.log(
                `✅ Payment transferred to artist and play recorded for song: ${id}`
              );
            })
            .catch((err: any) => {
              console.error(`⚠️ Failed to transfer payment to artist:`, err);
            });
        } else {
          // Direct payment - just record the play count
          suiService
            .recordPlay(id)
            .then(() => {
  ...
  fastify.post("/play/:id", async (request: X402Request, reply) => {
    const { id } = request.params as { id: string };
